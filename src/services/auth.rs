...
impl AuthService {
    ...
    pub async fn sign_out(&self) -> Result<(), String> {
        ...
        let sign_out_result = JsFuture::from(js_sys::Promise::from(sign_out_promise))
            .await
            .map_err(|e| {
                error!("Failed to await sign out: {:?}", e);
                format!("Failed to await sign out: {:?}", e)
            })?;

        let error = js_sys::Reflect::get(&sign_out_result, &JsValue::from_str("error"))
            .ok()
            .and_then(|e| {
                if e.is_null() {
                    None
        ...
        if let Some(error_msg) = error {
            error!("Sign out failed: {}", error_msg);
            return Err(format!("Sign out failed: {}", error_msg));
        }

        info!("Sign out promise resolved");
